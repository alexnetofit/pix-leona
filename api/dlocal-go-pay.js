...
export default async function handler(req, res) {
  ...
  const { account_id, email, qty, amount, offer_name, kind } = req.body || {};
  ...
  if (oneShot) {
    ...
    console.log('dlocal-go-pay: one_shot', created.status, data.id, title, amountReais);
    if (!created.ok || !data.id || !data.redirect_url) {
      logAssinaturaEvent(req, {
        ...
        email: buyerEmail,
        account_id: accountId,
        details: { qty: qtyN, kind: 'one_shot', status: created.status, error: created.body, title }
      });
    ...
    await rememberIntent({
      ...
      details: { kind: 'one_shot', offer_name: productName, order_id: title }
    });
    logAssinaturaEvent(req, {
  ...
  console.log('dlocal-go-pay: plan', qtyN, amountReais, plan.ok, plan.created, plan.plan?.id);
  if (!plan.ok || !plan.plan?.subscribe_url) {
    logAssinaturaEvent(req, {
      ...
      email: buyerEmail,
      account_id: accountId,
      details: { qty: qtyN, kind: 'subscription', error: plan.body, step: 'plan' }
    });
  ...
  await rememberIntent({
    ...
    details: { kind: 'subscription', offer_name: productName, order_id: title, plan_token: plan.plan.plan_token }
  });
