...
export async function findPagouCustomerByEmail(email) {
  ...
  const listed = await listPagouCustomers({ email: needle, limit: 20 });
  const rows = Array.isArray(listed.body?.data) ? listed.body.data : [];
  return rows.find((row) => String(row.email || '').trim().toLowerCase() === needle) || null;
}
...
export async function upsertPagouCustomer(payload) {
  ...
  const created = await createPagouCustomer(body);
  const data = payloadData(created.body);
  if (created.ok && data?.id) return { ok: true, status: created.status, data, created: true };

  const existing = await findPagouCustomerByEmail(payload.email);
  if (existing?.id) {
    const patched = await updatePagouCustomer(existing.id, body);
    const patchedData = payloadData(patched.body);
    return {
      ok: true,
      status: 200,
      data: patched.ok && patchedData?.id ? patchedData : existing,
      created: false,
      patched: patched.ok
    };
  }
