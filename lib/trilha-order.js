...
export function buildTrilhaCartOrder({ prizeIds = [], extras = {}, bumps = {}, acquiredIds = [] } = {}) {
  ...
  const items = [];
  const extrasByPrize = {};
  const hasPaidPrize = selected.some((prize) => !prize.prizeFree && prize.priceCents > 0 && !acquired.has(prize.id));
  let chargedFreePrize = false;
  for (const prize of selected) {
    ...
    if (!already) {
      const chargePrize = !prize.prizeFree
        ? prize.priceCents > 0
        : !hasPaidPrize && !chargedFreePrize && prize.priceCents > 0;
      if (chargePrize) {
        if (prize.prizeFree) chargedFreePrize = true;
        items.push({
