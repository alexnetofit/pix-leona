...
export function buildTrilhaCartOrder({
  ...
  let cheapLeft = Math.max(0, Math.floor(Number(cheapExtraRemaining) || 0));
  for (const prize of selected) {
    ...
    if (extra > 0) {
      if (!prize.extraUnitCents && !prize.priceCents) {
        return { ok: false, error: 'Esse prêmio não aceita unidades extras' };
      }
      const cheap = Math.min(extra, cheapLeft);
      const costly = extra - cheap;
      if (cheap > 0) {
        items.push({
          code: `trilha-${prize.id}-extra-trilha`,
          name: `${prize.title} (extra no preço da trilha)`,
          description: `${cheap} unidade(s) extra(s) no preço da trilha`,
          amount: prize.priceCents,
          quantity: cheap
        });
        cheapLeft -= cheap;
      ...
      if (costly > 0) {
        ...
        items.push({
          ...
          description: `${costly} unidade(s) extra(s)`,
          amount: prize.extraUnitCents,
          quantity: costly
        });
      }
...
export function trilhaThankYouUrl({ accountId, email, publicUrl } = {}) {
  const configured = String(publicUrl || process.env.TRILHA_PUBLIC_URL || 'https://client.leonaflow.com/trilha').replace(/\/+$/, '');
  const origin = configured.replace(/\/trilha$/i, '') || 'https://client.leonaflow.com';
  const qs = new URLSearchParams();
  if (accountId) qs.set('id', String(accountId));
  if (email) qs.set('email', String(email));
  const query = qs.toString();
  return query ? `${origin}/trilha-obrigado?${query}` : `${origin}/trilha-obrigado`;
}
