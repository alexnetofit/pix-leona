...
export async function findLeonaAccountByEmail(email, leonaToken) {
  ...
  try {
    ...
    if (r.status === 409) {
      ...
      const active = valid.filter((p) => isLeonaProfileActivelyPaid(p));
...
/**
 * Quantidade de slots do plano. `instances` no perfil agora é a lista de
 * conexões WhatsApp, não um número — não usar esse campo como quantidade.
 */
export function leonaProfileQuantity(profile) {
  const raw = profile?.starter_instances ?? profile?.subscription_instances ?? 0;
  const value = Number(raw);
  return Number.isInteger(value) && value >= 0 ? value : 0;
}

export function leonaTokenAdjustmentBody({ action, amount, note } = {}) {
  const act = String(action || '').trim().toLowerCase();
  if (act !== 'add' && act !== 'remove') {
    return { error: 'action inválida. Use add ou remove.' };
  }
  const value = typeof amount === 'number' ? amount : Number(String(amount ?? '').trim());
  if (!Number.isFinite(value)) return { error: 'amount inválido' };
  if (value <= 0) return { error: 'amount deve ser positivo' };
  const body = { action: act, amount: value };
  const text = note == null ? '' : String(note).trim();
  if (text) body.note = text.slice(0, 500);
  return { body };
}

/**
 * Credita ou debita a carteira de tokens. Não mexe em plano, vencimento nem status.
 * `remove` maior que o disponível não debita (a API responde 422).
 */
export async function adjustLeonaTokens(accountId, { action, amount, note } = {}, leonaToken) {
  if (!accountId || !leonaToken) return { ok: false, error: 'sem accountId/token' };
  const built = leonaTokenAdjustmentBody({ action, amount, note });
  if (built.error) return { ok: false, status: 400, error: built.error };
  const headers = leonaHeaders(leonaToken);
  try {
    const r = await fetch(`${LEONA_BASE}/accounts/${encodeURIComponent(accountId)}/tokens`, {
      method: 'POST',
      headers,
...
export function isLeonaProfileActivelyPaid(profile, now = new Date()) {
  if (String(profile?.subscription_status || '') !== 'active') return false;
  const end = profile.current_period_end || profile.due_date;
  if (!end) return true;
...
/**
 * Todas as contas ativas desse e-mail. 409 = dono em mais de uma conta —
 * o unique do /guru precisa contar cada uma.
 */
export async function listActiveLeonaAccountsByEmail(email, leonaToken) {
  if (!email || !leonaToken) return [];
  const headers = leonaHeaders(leonaToken);
  ...
  try {
    ...
    if (r.ok) {
      const profile = await r.json();
      return isLeonaProfileActivelyPaid(profile) ? [profile] : [];
    }
    ...
    return profiles.filter((profile) => isLeonaProfileActivelyPaid(profile));
  } catch (e) {
    console.error('listActiveLeonaAccountsByEmail: erro:', e.message);
    return [];
  }
}

export async function fetchExtraActiveLeonaAccounts(emails, leonaToken, { concurrency = 5 } = {}) {
  const unique = [...new Set((emails || []).map((value) => String(value || '').trim().toLowerCase()).filter(Boolean))];
  if (!unique.length || !leonaToken) return [];
  ...
  async function worker() {
    while (next < unique.length) {
      ...
      const accounts = await listActiveLeonaAccountsByEmail(email, leonaToken);
      const extra = Math.max(0, accounts.length - 1);
      if (extra > 0) {
        extras.push({
          email,
          extra,
          accountIds: accounts.map((profile) => profile.account_id)
        });
      }
    }
  }
