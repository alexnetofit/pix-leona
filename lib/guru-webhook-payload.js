...
/**
 * Renovação Guru (`invoice.type=cycle`) não pode derrubar instâncias que o
 * cliente já pagou/ganhou fora da Guru (ajuste Pagar.me, liberação manual).
 * Upgrade/downgrade explícito na Guru continua usando a qty da oferta.
 */
export function resolveGuruRenewalInstances({ invoiceType, offerQty, currentQty } = {}) {
  const offer = Number(offerQty);
  if (!Number.isFinite(offer) || offer <= 0) return null;
  const current = Number(currentQty) || 0;
  const type = String(invoiceType || '').toLowerCase();
  if (type === 'cycle' && current > offer) return current;
  return offer;
}
