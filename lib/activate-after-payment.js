import { getLeonaBillingProfile, updateLeonaBillingProfile } from './leona.js';
...
export async function activateLeonaAndCancelLegacy({
  ...
  if (!profile) {
    console.error('activate-after-payment: conta nao encontrada', accountId);
    return {
      ok: false,
      error: `conta ${accountId} nao encontrada`,
      profile: null,
      leona: null,
      guru: null,
      paddle: null
  ...
  const leona = await updateLeonaBillingProfile(accountId, {
    starter_instances: Number(qty),
    status: 'active',
    due_date: dueDate
  }, leonaToken);
  console.log('activate-after-payment: leona', accountId, qty, dueDate, leona.ok, leona.status);
  log.push({ step: 'leona', ok: leona.ok, status: leona.status, error: leona.body?.error || leona.error || null });

  const profileEmail = String(email || profile.user?.email || '').trim().toLowerCase();
  const guru = await cancelLegacyGuru({
    guruToken,
    profile,
    email: profileEmail,
    reason
  });
  const paddle = await cancelLegacyPaddle({
    email: profileEmail,
    reason
  });
...
async function cancelLegacyPaddle({ email, reason }) {
  if (!process.env.PADDLE_API_KEY || !process.env.PADDLE_ENVIRONMENT) {
