...
export async function listDlocalRefunds({ page = 0, size = 20, status } = {}) {
  const qs = new URLSearchParams({
    page: String(page || 0),
    size: String(Math.min(20, Number(size) || 20))
  });
  if (status) qs.set('status', String(status).trim().toUpperCase());
  return dlocalGoRequest('GET', `/v1/refunds?${qs}`);
}

export async function listAllDlocalRefunds({ status = 'SUCCESS', maxPages = 20 } = {}) {
  const rows = [];
  let pages = 0;
  for (let page = 0; page < maxPages; page++) {
    const found = await listDlocalRefunds({ page, size: 20, status });
    pages++;
    const batch = Array.isArray(found.body?.data) ? found.body.data : [];
    rows.push(...batch);
    const totalPages = Number(found.body?.totalPages ?? found.body?.total_pages ?? 0);
...
/** Exterior não gera checkout dLocal — a /assinatura manda pra Paddle. */
export function dlocalIntlRejection(region) {
  if (!isIntlRegion(region)) return null;
  return {
    status: 409,
    body: {
      error: 'Pagamento internacional agora é na Paddle. Use a aba Exterior da /assinatura.',
      code: 'INTL_USE_PADDLE'
    }
  };
}
