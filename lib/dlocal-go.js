...
export async function deactivateDlocalPlan(planId) {
  if (!planId) return { ok: false, status: 400, body: {} };
  return dlocalGoRequest('PATCH', `/v1/subscription/plan/${encodeURIComponent(planId)}/deactivate`);
}
...
export async function ensureDlocalPlan({
  ...
  const matches = rows.filter((row) => {
    ...
    const sameCurrency = String(row.currency || '').toUpperCase() === currencyCode;
    return sameName && sameCurrency && row.active !== false;
  });
  if (matches.length) {
    const exact = matches.find((row) => Number(row.amount) === amountN);
    const keep = exact || matches.slice().sort((a, b) => Number(b.id) - Number(a.id))[0];
    const extras = matches.filter((row) => Number(row.id) !== Number(keep.id));
    await Promise.all(extras.map((row) => deactivateDlocalPlan(row.id)));
