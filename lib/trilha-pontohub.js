...
export const PONTOHUB_LINKS = {
  '50k': '14c886e2-bad9-4ae4-98b2-20b5d95773f9',
  '100k': '9c8d7676-388e-4b64-a65b-68ce670941ca'
};
...
function line(code, qty, { campaign = false } = {}) {
  ...
  return Array.from({ length: qty }, (_, index) => ({
    ...
    linkId: campaign ? (PONTOHUB_LINKS[code] || null) : null
  }));
}
...
export function buildPontohubFulfillmentLines({ prizeId, extraQty = 0, bumps = {} } = {}) {
  ...
  const lines = [
    ...line(prizeId, 1, { campaign: true }),
    ...line(prizeId, extras, { campaign: false })
  ];
  for (const [bumpId, qty] of Object.entries(bumps || {})) {
    lines.push(...line(bumpId, Number(qty) || 0, { campaign: false }));
  }
  return lines;
}
