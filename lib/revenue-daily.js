...
export async function collectRevenuePlan(plan, { guruToken, includeSnapshot } = {}) {
  ...
  if ((guruDays.length || wantsSnapshot) && !guruToken) throw new Error('GURU_TOKEN não configurado');
...
export function summarizeRange(start, end, rows, snapshot) {
  ...
  const summedSubscribers = (guruSnapshot || paddleSnapshot || pagouSnapshot || dlocalSnapshot)
