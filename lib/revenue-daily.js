...
export const PLATFORMS = ['guru', 'paddle', 'pagou', 'dlocal'];
...
/**
 * Busca so as plataformas do plano. Se so falta Pagou/dLocal, Guru e Paddle
 * ficam quietas — e o que deixava a tela /guru lenta depois da inclusao delas.
 */
export async function collectRevenuePlan(plan, { guruToken, includeSnapshot } = {}) {
  ...
  const days = [...new Set([...guruDays, ...paddleDays, ...pagouDays, ...dlocalDays])].sort();
  if (!days.length) return { rows: [], days: [], snapshot: null };
  if (guruDays.length && !guruToken) throw new Error('GURU_TOKEN não configurado');
  ...
  const wantsSnapshot = includeSnapshot ?? Object.values(snapshotFlags).some(Boolean);

  const [guruByDay, paddleResult, pagouResult, dlocalResult, snapshot] = await Promise.all([
    ...
    wantsSnapshot ? collectSnapshot(guruToken, snapshotFlags) : Promise.resolve(null)
  ]);
...
async function collectSnapshot(guruToken, flags = { guru: true, paddle: true, pagou: true, dlocal: true }) {
  const [guruCount, paddle, pagou, dlocal] = await Promise.all([
    flags.guru ? fetchGuruActiveSubscribers(guruToken) : Promise.resolve(null),
    flags.paddle ? fetchPaddleSubscriberSnapshot() : Promise.resolve(null),
    flags.pagou ? fetchPagouSubscriberSnapshot() : Promise.resolve(null),
    flags.dlocal ? fetchDlocalSubscriberSnapshot() : Promise.resolve(null)
  ]);
  const snapshot = {};
  if (flags.guru) snapshot.guru = { count: guruCount, recurring: null, prepaid: null };
  if (flags.paddle) {
    snapshot.paddle = { count: paddle.count, recurring: paddle.recurring, prepaid: paddle.prepaid };
  }
  if (flags.pagou) {
    snapshot.pagou = { count: pagou.count, recurring: pagou.recurring, prepaid: pagou.prepaid };
  }
  if (flags.dlocal) {
    snapshot.dlocal = { count: dlocal.count, recurring: dlocal.recurring, prepaid: dlocal.prepaid };
  }
  return snapshot;
}
...
export function summarizeRange(start, end, rows, snapshot) {
  ...
  const brlRows = [...guruRows, ...paddleRows, ...dlocalRows];
  ...
  const subscriberTotal = (guruSnapshot || paddleSnapshot || pagouSnapshot || dlocalSnapshot)
  ...
  return {
    ...
    platforms: { guru, paddle, pagou, dlocal },
