...
/**
 * Busca so as plataformas do plano. Se so falta Pagou, Guru e Paddle ficam
 * quietas — e o que deixava a tela /guru lenta depois da inclusao da Pagou.
 */
export async function collectRevenuePlan(plan, { guruToken, includeSnapshot } = {}) {
  ...
  const days = [...new Set([...guruDays, ...paddleDays, ...pagouDays])].sort();
...
export async function collectRevenueDays(days, { guruToken, includeSnapshot } = {}) {
  ...
  return collectRevenuePlan(
    { guru: days, paddle: days, pagou: days },
...
async function collectSnapshot(guruToken, flags = { guru: true, paddle: true, pagou: true }) {
