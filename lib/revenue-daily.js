...
export const PLATFORMS = ['guru', 'paddle'];
...
function buildRows(days, guruByDay, paddleByDay, snapshot, today) {
  const syncedAt = new Date().toISOString();
  const rows = [];

  for (const day of days) {
    for (const platform of PLATFORMS) {
      const totals = (platform === 'guru' ? guruByDay : paddleByDay).get(day)
        || emptyTotals();
...
export async function collectRevenueDays(days, { guruToken, includeSnapshot } = {}) {
  ...
  const [guruByDay, paddleResult, snapshot] = await Promise.all([
  ...
  return {
    rows: buildRows(days, guruByDay, paddleResult.byDay, snapshot, today),
    days,
    snapshot
  };
}
...
async function collectSnapshot(guruToken) {
  const [guruCount, paddle] = await Promise.all([
...
export function summarizeRange(start, end, rows, snapshot) {
  ...
  const combined = sumTotals(inRange);
  ...
  const subscriberTotal = (guruSnapshot || paddleSnapshot)
    ? (guruSnapshot?.count || 0) + (paddleSnapshot?.count || 0)
    : null;
