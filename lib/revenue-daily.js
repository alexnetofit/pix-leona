...
import {
  ...
  listPaidDlocalCheckoutEmails,
  listPaidCheckoutAccountRows,
...
import { fetchExtraActiveLeonaAccounts } from './leona.js';
...
export async function readSubscriberSnapshot() {
  ...
  for (const row of rows) {
    ...
    if (snapshot.unique == null && row.unique_subscribers != null) {
      snapshot.unique = Number(row.unique_subscribers);
    }
  }
  return snapshot;
}
...
function buildPlatformRows(days, platform, byDay, snapshot, today) {
  ...
  return days.map(day => {
    ...
    return {
      ...
      unique_subscribers: day === today && snapshot?.unique != null
        ? Number(snapshot.unique)
        : null,
      synced_at: syncedAt
    };
  });
...
async function collectCheapSnapshot(flags = {}) {
  ...
  const today = brtToday();
  const dlocalEmails = flags.pagarme
    ? await listPaidDlocalCheckoutEmails({
      sinceIso: `${shiftDays(today, -DLOCAL_ACTIVE_DAYS)}T00:00:00.000Z`,
  ...
  const [pagou, pagarme] = await Promise.all([
    ...
    flags.pagarme
      ? fetchPagarmeSubscriberSnapshot({ occupiedEmails: dlocalEmails })
      : Promise.resolve(null)
  ]);
  const guru = snapshotFromRow(existing.guru);
  const paddle = snapshotFromRow(existing.paddle);
  const pagouRow = pagou
    ? { count: pagou.count, recurring: pagou.recurring, prepaid: pagou.prepaid }
    : snapshotFromRow(existing.pagou);
  const dlocal = snapshotFromRow(existing.dlocal);
  const pagarmeRow = pagarme
    ? { count: pagarme.count, recurring: pagarme.recurring, prepaid: pagarme.prepaid }
    : snapshotFromRow(existing.pagarme);
  const intentExtra = extraUniqueFromAccountGroups(
    await listPaidCheckoutAccountRows({
      sinceIso: `${shiftDays(today, -DLOCAL_ACTIVE_DAYS)}T00:00:00.000Z`,
      untilIso: `${shiftDays(today, 2)}T00:00:00.000Z`
    })
  ).extra;
  const prevSum = subscriberCardSum({
    guru: snapshotFromRow(existing.guru),
    paddle: snapshotFromRow(existing.paddle),
    pagou: snapshotFromRow(existing.pagou),
    dlocal: snapshotFromRow(existing.dlocal),
    pagarme: snapshotFromRow(existing.pagarme)
  });
  const next = {
    guru,
    paddle,
  ...
  const leonaExtra = preservedUniqueExtras(existing.unique, prevSum, intentExtra);
  return {
    unique: cheapSnapshotUnique({
...
async function collectSnapshot(guruToken) {
  ...
  const today = brtToday();
  const intentGroups = extraUniqueFromAccountGroups(
    await listPaidCheckoutAccountRows({
      sinceIso: `${shiftDays(today, -DLOCAL_ACTIVE_DAYS)}T00:00:00.000Z`,
      untilIso: `${shiftDays(today, 2)}T00:00:00.000Z`
    })
  );
  const leonaToken = process.env.LEONA_BILLING_TOKEN || process.env.LEONA_LOOKUP_TOKEN;
  const leonaExtras = await fetchExtraActiveLeonaAccounts(
    [...new Set([...pagarmeEmails, ...intentGroups.emails])],
    leonaToken
  );
  const extraByEmail = new Map(intentGroups.extras.map((row) => [row.email, row.extra]));
  for (const row of leonaExtras) {
    extraByEmail.set(row.email, Math.max(extraByEmail.get(row.email) || 0, row.extra));
  }
  const extraUnique = [...extraByEmail.values()].reduce((sum, value) => sum + value, 0);
  ...
  return {
    ...
    dlocal: {
      count: dlocal.count,
      recurring: dlocal.recurring,
      prepaid: dlocal.prepaid
    },
    pagarme: {
      count: pagarme.count,
      recurring: pagarme.recurring,
      prepaid: pagarme.prepaid
    }
  };
}
