...
/**
 * Quais e-mails da lista ainda têm assinatura Leona ativa na Guru.
 * Usado pro painel não contar de novo quem já entrou no total da Guru.
 */
export async function filterEmailsWithActiveGuru(emails, guruToken, { concurrency = 4 } = {}) {
  const unique = [...new Set((emails || []).map(normalizeEmail).filter(Boolean))];
  if (!unique.length || !guruToken) return [];

  const hits = new Array(unique.length).fill(false);
  let next = 0;
  async function worker() {
    while (next < unique.length) {
      const index = next++;
      const found = await findGuruActiveSubscriptionsByEmail(unique[index], guruToken);
      hits[index] = Array.isArray(found) && found.length > 0;
    }
  }
  await Promise.all(
    Array.from({ length: Math.min(concurrency, unique.length) }, () => worker())
  );
  return unique.filter((_, index) => hits[index]);
}
...
/** Prazo maximo (dias) pra solicitar reembolso de transacao Guru paga. */
export const GURU_REFUND_WINDOW_DAYS = 7;

function guruTsToIso(ts) {
  const n = Number(ts);
  if (!Number.isFinite(n) || n <= 0) return null;
  return new Date(n * 1000).toISOString();
}
...
/**
 * Data efetiva de pagamento de uma transacao Guru.
 * Prioriza confirmed_at; se ausente, usa charge_at da fatura paga
 * (campo "Cobrado em" no painel Guru).
 */
export function resolveGuruTransactionPaidAt(t) {
  if (!t) return null;
  const confirmed = guruTsToIso(t.confirmed_at);
  if (confirmed) return confirmed;
  const chargeAt = t.invoice?.charge_at;
  if (chargeAt && (t.status === 'approved' || t.invoice?.status === 'paid')) {
    return `${String(chargeAt).slice(0, 10)}T12:00:00.000Z`;
  }
  return guruTsToIso(t.status_at);
}

export function isGuruTransactionRefundable(t, opts = {}) {
  const windowDays = opts.windowDays ?? GURU_REFUND_WINDOW_DAYS;
  if (!t || t.status !== 'approved') return false;
  const paidAt = resolveGuruTransactionPaidAt(t);
  return isPaidAtWithinRefundWindow(paidAt, { windowDays });
}

/** Valida janela de reembolso a partir de paid_at ISO (ex.: snapshot). */
export function isPaidAtWithinRefundWindow(paidAt, opts = {}) {
  const windowDays = opts.windowDays ?? GURU_REFUND_WINDOW_DAYS;
  if (!paidAt) return false;
  const paidMs = new Date(paidAt).getTime();
  if (!Number.isFinite(paidMs)) return false;
  return (Date.now() - paidMs) <= windowDays * 86400000;
}

/**
 * Reembolsa uma transacao Guru via POST /transactions/:id/refund.
 *
 * A Guru aceita reembolso de transacoes com status `approved` (paga). O
 * endpoint retorna 422 se a transacao ja foi reembolsada/estornada ou
 * nao esta em estado reembolsavel.
 *
 * Aceita o `id` (UUID interno) ou o `transaction_code` da Guru.
 *
 * Devolve { ok, status, body } padronizado.
 */
export async function refundGuruTransaction(transactionId, guruToken, opts = {}) {
  if (!transactionId || !guruToken) {
    return { ok: false, error: 'sem transaction_id ou token' };
  }
  const { reason = 'Reembolso solicitado pelo cliente', comment } = opts;
  const commentBody = (comment || reason || 'Reembolso solicitado pelo cliente').toString().trim();
  try {
    const r = await fetch(
      `${GURU_BASE}/transactions/${encodeURIComponent(transactionId)}/refund`,
      {
        method: 'POST',
        headers: guruHeaders(guruToken),
