...
export async function fetchPagarmeSubscriberSnapshot({ occupiedEmails = [] } = {}) {
  ...
  if (pagarmeConfigured()) {
    const [listed, subs] = await Promise.all([
      listAllPagarmeOrders({
        status: 'paid',
      ...
      listAllPagarmeSubscriptions({ status: 'active', maxPages: 20 })
    ]);
    pages += listed.pages + subs.pages;
    orders = listed.rows;
    ...
    orders = intents.map(intentAsPagarmeOrder);
  }
