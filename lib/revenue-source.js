 ...
 *   - Em ambos, o reembolso e atribuido ao dia da venda original.
 */
import { GURU_BASE, LEONA_GURU_PRODUCT_ID, guruHeaders } from './guru.js';
import { paddleRequest } from './paddle-client.js';
import { getPagouTransaction, listPagouSubscriptions, listPagouTransactions, pagouConfigured } from './pagou.js';
...
function pagouUsdCents(tx) {
  const usd = intCents(tx.amount || tx.fee?.net_amount);
  return usd > 0 ? usd : 0;
}

function pagouNetUsdCents(tx) {
  const settled = intCents(tx.settlements?.summary?.total_net_amount);
  if (settled > 0) return settled;
  const net = intCents(tx.fee?.net_amount);
  if (net > 0) return net;
  return pagouUsdCents(tx);
}

function pagouBrlCents(tx) {
  return intCents(tx.paid_amount || tx.base_price);
}
...
async function hydratePagouTransactions(rows) {
  ...
  const details = await mapWithConcurrency(unique, 5, async (id) => {
    ...
    if (listed && (listed.amount || listed.paid_amount || listed.fee)) {
      return listed;
    }
...
/**
 * Totais da Pagou por dia, em centavos de USD (o settlement da LLC).
 * `transactions_scanned` guarda o bruto em BRL só pra barra de participação.
 */
export async function fetchPagouDailyTotals(days) {
  ...
  for (const tx of details) {
    ...
    const day = brtDayOf(tx.paid_at || tx.created_at || '');
    if (!day || !wanted.has(day)) continue;
    const usd = pagouUsdCents(tx);
    const net = pagouNetUsdCents(tx);
    const brl = pagouBrlCents(tx);
    if (usd <= 0 && brl <= 0) continue;

    const totals = byDay.get(day) || emptyTotals();
    totals.transactions_scanned += brl;
    totals.source_pages = listed.pages;
    if (PAGOU_PAID.has(status)) {
      totals.gross_cents += usd;
      totals.net_cents += net;
      totals.sales_count++;
    } else if (PAGOU_REFUND.has(status)) {
      totals.refund_gross_cents += usd;
      totals.refund_net_cents += net;
      totals.refund_count++;
    }
    byDay.set(day, totals);
...
export async function fetchPagouSubscriberSnapshot() {
  ...
  const listed = await listPagouSubscriptions({ status: 'active' });
  const recurring = Array.isArray(listed.body?.data) ? listed.body.data.length : 0;
  const today = brtToday();
  const recent = await listPagouTransactions({
    status: 'paid',
    dateFrom: brtBoundary(shiftDays(today, -PAGOU_ACTIVE_DAYS))
  });
  const emails = recent.rows
    .map((tx) => String(tx.buyer?.email || tx.customer?.email || tx.email || '').toLowerCase())
    .filter(Boolean);
  let prepaid = new Set(emails);
  if (!prepaid.size && recent.rows.length) {
    const details = await hydratePagouTransactions(recent.rows);
    prepaid = new Set(
      details
        .map((tx) => String(tx.buyer?.email || tx.customer?.email || tx.email || '').toLowerCase())
        .filter(Boolean)
