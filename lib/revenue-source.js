...
export async function fetchPagarmeSubscriberSnapshot({ occupiedEmails = [] } = {}) {
  ...
  // A listagem da Stone mistura Guru + /assinatura e corta em 80 páginas.
  // Cada cron pega um recorte diferente e o card único pisca (2426↔2421).
  // Os checkouts da /assinatura já estão no Supabase — essa lista é estável.
  if (sbConfigured()) {
    const intents = await listPagarmeAssinaturaIntents({
      sinceIso: `${shiftDays(today, -PAGARME_ACTIVE_DAYS)}T00:00:00.000Z`,
      untilIso: `${shiftDays(today, 2)}T00:00:00.000Z`,
      statuses: ['paid']
    });
    orders = collapsePagarmeRepeatIntents(intents).map(intentAsPagarmeOrder);
  } else if (pagarmeConfigured()) {
    const listed = await listAllPagarmeOrders({
      status: 'paid',
      createdSince: `${shiftDays(today, -PAGARME_ACTIVE_DAYS)}T00:00:00.000Z`,
      createdUntil: `${shiftDays(today, 2)}T00:00:00.000Z`,
      maxPages: 80
    });
    pages += listed.pages;
    orders = listed.rows;
