...
import { pagarmeOrderLooksPaid } from './pagarme.js';
...
export function isPagarmeLeonaOrder(order = {}) {
  ...
  // mesma conta Stone; descrição "Leona Flow" ou kind=subscription não vale.
  return /^leona-/i.test(String(order?.code || ''));
}
...
export async function fetchPagarmeDailyTotals(days) {
  if (!days.length || !sbConfigured()) return { byDay: new Map(), pages: 0 };
...
export async function fetchPagarmeSubscriberSnapshot({ occupiedEmails = [] } = {}) {
  ...
  const occupied = new Set((occupiedEmails || []).map(normalizeSubscriberEmail).filter(Boolean));
  const orders = intents
    .map(intentAsPagarmeOrder)
    .filter((order) => {
      const email = pagarmePartyEmail(order);
      return email && !occupied.has(email);
    });
  return {
    ...buildPagarmeSubscriberSnapshot({ orders }),
    pages: 0
  };
}
