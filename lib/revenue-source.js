 ...
 *     diferenca entra em refund_* sem sair do gross. Checkout novo (produto
 *     "Leona Instances" ou codigo leona-…) entra junto. Moeda estrangeira
 *     usa o payout em USD da propria Paddle, convertido pra BRL pelo PTAX
 *     do dia da venda.
 ...
 *   - Pagar.me: so o checkout da /assinatura (code leona-…) na API Stone.
 *     Cobranca que a Guru pediu (UUID) nao entra — ja esta no card Guru.
 ...
 */
export function extraUniqueFromAccountGroups(rows = []) {
  const byEmail = new Map();
  for (const row of rows) {
    const email = normalizeSubscriberEmail(row.email);
    const accountId = String(row.account_id || '').trim();
    if (!email || !accountId) continue;
    if (!byEmail.has(email)) byEmail.set(email, new Set());
    byEmail.get(email).add(accountId);
  }
  const extras = [];
  for (const [email, ids] of byEmail) {
    const more = Math.max(0, ids.size - 1);
    if (more > 0) extras.push({ email, extra: more });
  }
  return {
    extra: extras.reduce((sum, row) => sum + row.extra, 0),
    emails: extras.map((row) => row.email),
    extras
  };
}

export function subscriberCardSum(snapshot = {}) {
  return (Number(snapshot.guru?.count) || 0)
    + (Number(snapshot.paddle?.count) || 0)
    + (Number(snapshot.pagou?.count) || 0)
    + (Number(snapshot.dlocal?.count) || 0)
    + (Number(snapshot.pagarme?.count) || 0);
}

export function preservedUniqueExtras(unique, cardSum, alreadyCounted = 0) {
  if (unique == null) return 0;
  return Math.max(0, (Number(unique) || 0) - (Number(cardSum) || 0) - (Number(alreadyCounted) || 0));
}

/**
 * O sync barato nao pode somar os cards. Guru/Paddle ficam no valor bruto e
 * Pagar.me tambem — somar de novo conta duas vezes quem migrou. Se ja existe
 * um unique deduplicado, ele fica; a rodada completa e que refaz.
 */
export function cheapSnapshotUnique({ existingUnique, next, intentExtra = 0, leonaExtra = 0 } = {}) {
  if (existingUnique != null && Number.isFinite(Number(existingUnique))) {
    return Number(existingUnique);
  }
  return subscriberCardSum(next) + Number(intentExtra || 0) + Number(leonaExtra || 0);
}
...
function paddleLabelIsLeona(value) {
  const name = String(value || '').trim().toLowerCase();
  return name === 'leona flow' || name === 'leona instances';
}

function paddleCustomIsLeona(custom) {
  if (!custom || typeof custom !== 'object') return false;
  if (String(custom.leona_account_id || '').trim()) return true;
  if (custom.leona_billing === true || String(custom.leona_billing).toLowerCase() === 'true') {
    return true;
  }
  return /^leona-/i.test(String(custom.leona_order_code || ''));
}

function paddlePriceIds(explicit) {
...
export function isPaddleLeonaTransaction(transaction, priceIds) {
  const configured = new Set(paddlePriceIds(priceIds));
  const items = Array.isArray(transaction?.items) ? transaction.items : [];
  const lineItems = Array.isArray(transaction?.details?.line_items)
    ? transaction.details.line_items
    : [];
  if (items.some(item => configured.has(item?.price?.id))) return true;
  if (lineItems.some(item => configured.has(item?.price_id))) return true;
  const labels = [
    ...items.flatMap(item => [
      item?.product?.name,
      item?.price?.name,
    ...
    ...lineItems.map(item => item?.product?.name)
  ];
  if (labels.some(paddleLabelIsLeona)) return true;
  return paddleCustomIsLeona(transaction?.custom_data);
}

function paddleTransactionIsLeona(transaction) {
  return isPaddleLeonaTransaction(transaction);
}
...
function paddleSubscriptionIsLeona(subscription) {
  ...
  return items.some(item =>
    ...
    || paddleLabelIsLeona(item?.price?.product?.name)
    || paddleLabelIsLeona(item?.price?.name)
    || paddleLabelIsLeona(item?.product?.name)
  ) || paddleCustomIsLeona(subscription?.custom_data);
}

const ptaxByDay = new Map();

/** PTAX venda do Banco Central. Fim de semana e feriado caem no dia útil anterior. Sem PTAX, usa o Frankfurter do mesmo dia. */
export async function usdToBrlPtax(day) {
  if (!isValidDay(day)) return null;
  if (ptaxByDay.has(day)) return ptaxByDay.get(day);
  for (let back = 0; back < 7; back++) {
    const cursor = shiftDays(day, -back);
    const [year, month, date] = cursor.split('-');
    const url = 'https://olinda.bcb.gov.br/olinda/servico/PTAX/versao/v1/odata/'
      + `CotacaoDolarDia(dataCotacao=@dataCotacao)?@dataCotacao='${month}-${date}-${year}'`
      + '&$top=1&$orderby=dataHoraCotacao desc&$format=json';
    try {
      const response = await fetch(url);
      if (!response.ok) continue;
      const body = await response.json();
      const rate = Number(body?.value?.[0]?.cotacaoVenda);
      if (rate > 0) {
        ptaxByDay.set(day, rate);
        return rate;
      }
    } catch {
      // tenta o dia útil anterior
    }
  }
  try {
    const response = await fetch(`https://api.frankfurter.app/${day}?from=USD&to=BRL`);
    if (response.ok) {
      const body = await response.json();
    ...
    // sem câmbio: a venda estrangeira fica de fora em vez de virar real inventado
  }
  ptaxByDay.set(day, null);
  return null;
}

/**
 * Bruto e líquido em centavos de BRL. Venda em real usa o total cobrado.
 * Outra moeda usa o payout em USD da Paddle vezes o câmbio informado.
 * Reembolso não sai do bruto: volta em refundGross/refundNet.
 */
export function paddleSaleBrlCents(transaction, usdToBrlRate = null) {
  const totals = transaction?.details?.totals || {};
  const payout = transaction?.details?.payout_totals || {};
  const adjusted = transaction?.details?.adjusted_totals || totals;
  const currency = String(totals.currency_code || transaction?.currency_code || 'BRL').toUpperCase();
  const payoutCurrency = String(payout.currency_code || '').toUpperCase();

  let gross = 0;
  let net = 0;
  if (currency === 'BRL') {
    gross = intCents(totals.grand_total);
    net = intCents(totals.earnings);
  } else if (payoutCurrency === 'BRL') {
    gross = intCents(payout.grand_total);
    net = intCents(payout.earnings);
  } else if (payoutCurrency === 'USD' && Number(usdToBrlRate) > 0) {
    const rate = Number(usdToBrlRate);
    gross = Math.round(intCents(payout.grand_total) * rate);
    net = Math.round(intCents(payout.earnings) * rate);
  } else {
    return null;
  }
  ...
  let refundNet = 0;
  if (currency === 'BRL') {
    refundGross = Math.max(0, gross - intCents(adjusted.grand_total));
    refundNet = Math.max(0, net - intCents(adjusted.earnings));
  } else {
    const chargeGross = intCents(totals.grand_total);
    const chargeLeft = intCents(adjusted.grand_total);
    if (chargeGross > 0 && chargeLeft < chargeGross) {
      const ratio = (chargeGross - chargeLeft) / chargeGross;
      refundGross = Math.round(gross * ratio);
      refundNet = Math.round(net * ratio);
    }
  }
  return { gross, net, refundGross, refundNet };
}
...
export async function fetchPaddleDailyTotals(days) {
  ...
  for (const transaction of rows) {
    ...
    let rate = null;
    if (currency !== 'BRL') {
      if (!rateByDay.has(day)) rateByDay.set(day, await usdToBrlPtax(day));
      rate = rateByDay.get(day);
    }
    const money = paddleSaleBrlCents(transaction, rate);
    if (!money) continue;

    const totals = byDay.get(day) || emptyTotals();
    totals.gross_cents += money.gross;
    totals.net_cents += money.net;
    totals.sales_count++;
    totals.transactions_scanned++;
    if (money.refundGross > 0) {
      totals.refund_gross_cents += money.refundGross;
      totals.refund_net_cents += money.refundNet;
      totals.refund_count++;
    }
...
function pagarmeOrderItems(order = {}) {
  if (Array.isArray(order.items)) return order.items;
  if (Array.isArray(order.order?.items)) return order.order.items;
  return [];
}

function pagarmeItemDescription(order = {}) {
  return pagarmeOrderItems(order).map((item) => String(item?.description || '')).join(' ');
}

export const PAGARME_REPEAT_WINDOW_MS = 60_000;

function pagarmeRepeatKey(intent = {}) {
  const kind = String(intent.details?.kind || '').toLowerCase();
  return [
    String(intent.account_id || '').trim(),
    String(intent.amount_cents || ''),
    kind
  ].join('|');
}

/**
 * Clique duplo no checkout: mesmo conta + valor + tipo em até 60s.
 * O dinheiro pode ter entrado duas vezes; a contagem do painel fica uma.
 */
export function collapsePagarmeRepeatIntents(intents = [], { windowMs = PAGARME_REPEAT_WINDOW_MS } = {}) {
  const sorted = [...intents].sort((a, b) => {
    const ta = new Date(a.created_at || a.paid_at || 0).getTime();
    const tb = new Date(b.created_at || b.paid_at || 0).getTime();
    return ta - tb;
  });
  const kept = [];
  for (const intent of sorted) {
    const accountId = String(intent.account_id || '').trim();
    const amount = String(intent.amount_cents ?? '');
    if (!accountId || !amount) {
      kept.push(intent);
      continue;
    }
    const key = pagarmeRepeatKey(intent);
    const t = new Date(intent.created_at || intent.paid_at || 0).getTime();
    const dup = Number.isFinite(t) && kept.some((prev) => {
      if (pagarmeRepeatKey(prev) !== key) return false;
      const prevT = new Date(prev.created_at || prev.paid_at || 0).getTime();
      return Number.isFinite(prevT) && Math.abs(t - prevT) <= windowMs;
    });
    if (dup) continue;
    kept.push(intent);
  }
...
function pagarmeOrderRepeatKey(order = {}) {
  const code = String(order?.code || order?.order?.code || '');
  const account = (/^leona-(\d+)/i.exec(code) || [])[1] || pagarmePartyEmail(order);
  const kind = isPagarmeOneShotOrder(order) ? 'one_shot' : 'subscription';
  return `${account}|${pagarmeGrossCents(order)}|${kind}`;
}

export function collapsePagarmeRepeatOrders(orders = [], { windowMs = PAGARME_REPEAT_WINDOW_MS } = {}) {
  const sorted = [...orders].sort((a, b) => {
    const ta = new Date(a?.charges?.[0]?.paid_at || a?.created_at || 0).getTime();
    const tb = new Date(b?.charges?.[0]?.paid_at || b?.created_at || 0).getTime();
    return ta - tb;
  });
  const kept = [];
  for (const order of sorted) {
    const key = pagarmeOrderRepeatKey(order);
    const account = key.split('|')[0];
    if (!account) {
      kept.push(order);
      continue;
    }
    const t = new Date(order?.charges?.[0]?.paid_at || order?.created_at || 0).getTime();
    const dup = Number.isFinite(t) && kept.some((prev) => {
      if (pagarmeOrderRepeatKey(prev) !== key) return false;
      const prevT = new Date(prev?.charges?.[0]?.paid_at || prev?.created_at || 0).getTime();
      return Number.isFinite(prevT) && Math.abs(t - prevT) <= windowMs;
    });
    if (dup) continue;
    kept.push(order);
...
export function isPagarmeLeonaOrder(order = {}) {
  // Só o checkout da /assinatura. A Guru também liquida na mesma conta
  // Stone; UUID da transação Guru não vale — já entra no card da Guru.
  const code = String(order?.code || order?.order?.code || '');
  if (/^leona-/i.test(code)) return true;
  // Payment link antigo da /assinatura: o code vira o pl_, o item é Leona.
  if (!/^pl_/i.test(code)) return false;
  const meta = order.metadata || order.order?.metadata || {};
  const guruish = `${meta.module_name || ''} ${meta.platform_integration || ''}`.toLowerCase();
  if (guruish.includes('guru')) return false;
  return /leona/i.test(pagarmeItemDescription(order));
}
...
export function isPagarmeOneShotOrder(order = {}) {
  ...
  if (/-(prorata)$/i.test(String(order?.code || ''))) return true;
  return /ajuste\s+leona/i.test(pagarmeItemDescription(order));
}

export function isPagarmeTokenOrder(order = {}) {
  return /^leona-tokens-/i.test(String(order?.code || order?.order?.code || ''));
}
...
/**
 * A Stone mistura Guru + /assinatura e corta em 80 paginas (2400 pedidos).
 * Um intervalo unico de 32 dias ja passa disso (~2.600), entao o snapshot
 * parte a janela em fatias. created_until e exclusivo.
 */
export function pagarmeChunkWindows(sinceDay, untilDay, chunkDays = 7) {
  if (!isValidDay(sinceDay) || !isValidDay(untilDay) || sinceDay >= untilDay) return [];
  const size = Math.max(1, Number(chunkDays) || 7);
  const windows = [];
  let cursor = sinceDay;
  while (cursor < untilDay) {
    const next = shiftDays(cursor, size);
    const end = next < untilDay ? next : untilDay;
    windows.push({
      createdSince: `${cursor}T00:00:00.000Z`,
      createdUntil: `${end}T00:00:00.000Z`
    });
    cursor = end;
  }
  return windows;
...
export function mergePagarmeOrdersById(...lists) {
  const seen = new Set();
  const rows = [];
  for (const list of lists) {
    for (const order of list || []) {
      const id = String(order?.id || order?.code || '');
      if (!id || seen.has(id)) continue;
      seen.add(id);
      rows.push(order);
    }
...
export async function listPagarmeOrdersChunked({
  sinceDay,
  untilDay,
  status = 'paid',
  chunkDays = 7,
  ...
  const windows = pagarmeChunkWindows(sinceDay, untilDay, chunkDays);
  const rows = [];
  let pages = 0;
  for (const window of windows) {
    ...
    if (listed.truncated) {
      const error = new Error(
        `Pagar.me cortou a listagem em ${maxPagesPerChunk} páginas (${window.createdSince}–${window.createdUntil})`
      );
      error.status = 502;
      throw error;
    }
  ...
  return { rows: mergePagarmeOrdersById(rows), pages };
}
...
async function listPagarmeAssinaturaIntents({ sinceIso, untilIso, statuses = ['paid'] } = {}) {
  ...
  // or_/pl_ sao os ids da Pagar.me. Evita jsonb `cs` e nao mistura com a dLocal.
  const [orders, links] = await Promise.all([
    sbSelectWhere('dlocal_checkout_intents', { ...filters, like: { dlocal_payment_id: 'or_*' } }),
    sbSelectWhere('dlocal_checkout_intents', { ...filters, like: { dlocal_payment_id: 'pl_*' } })
  ]);
  const wanted = new Set((statuses || []).map((status) => String(status).toLowerCase()));
  return [...orders, ...links].filter((row) =>
    row?.details?.provider === 'pagarme'
    && wanted.has(String(row.status || '').toLowerCase())
  );
}

export function buildPagarmeSubscriberSnapshot({ orders = [], subscriptions = [] } = {}) {
  const recurring = new Set();
  const prepaid = new Set();
  for (const sub of subscriptions) {
    if (!pagarmeSubscriptionActive(sub)) continue;
    const email = String(sub.customer?.email || sub.email || '').trim().toLowerCase();
    if (email) recurring.add(email);
  }
  for (const order of orders) {
    ...
    if (isPagarmeTokenOrder(order)) continue;
    const email = pagarmePartyEmail(order);
    if (!email) continue;
    if (recurring.has(email)) continue;
  ...
  return {
    // Só ciclo novo / assinatura nativa vira cabeça. Ajuste e token já
    // estão em outra plataforma ou não são assinante.
...
function applyPagarmeOrderToDay(byDay, wanted, order) {
  if (!isPagarmeLeonaOrder(order)) return;
  const day = pagarmePaymentDay(order);
  if (!day || !wanted.has(day)) return;
  const gross = pagarmeGrossCents(order);
...
export async function fetchPagarmeDailyTotals(days) {
  ...
  if (pagarmeConfigured()) {
    // Uma janela só corta em 80 páginas (2400 pedidos). O rescan de 30 dias
    // passa disso, a API devolve os mais novos e o dia que saiu do corte é
    // gravado como zero — o mês encolhe toda madrugada. A mesma fatia de 7
    // dias do snapshot cabe na página.
    const [paid, canceled] = await Promise.all([
      listPagarmeOrdersChunked({
        sinceDay: window.createdSince,
        untilDay: window.createdUntil,
        status: 'paid'
      }),
      listPagarmeOrdersChunked({
        sinceDay: window.createdSince,
    ...
    pages += paid.pages + canceled.pages;
    const seen = new Set();
    const orders = [];
    ...
    for (const order of collapsePagarmeRepeatOrders(orders)) {
      applyPagarmeOrderToDay(byDay, wanted, order);
    }
  } else if (sbConfigured()) {
    const intents = await listPagarmeAssinaturaIntents({
      sinceIso: `${window.createdSince}T00:00:00.000Z`,
      untilIso: `${window.createdUntil}T00:00:00.000Z`,
      statuses: ['paid', 'refunded', 'canceled', 'cancelled']
    });
    for (const intent of collapsePagarmeRepeatIntents(intents)) {
      applyPagarmeOrderToDay(byDay, wanted, intentAsPagarmeOrder(intent));
    }
  }
...
export async function listPaidCheckoutAccountRows({
  sinceIso,
  untilIso
} = {}) {
  if (!sbConfigured()) return [];
  const rows = await sbSelectWhere('dlocal_checkout_intents', {
    eq: { status: 'paid' },
    ...(sinceIso ? { gte: { created_at: sinceIso } } : {}),
    ...(untilIso ? { lt: { created_at: untilIso } } : {}),
    order: 'created_at.asc',
    limit: 5000
  });
  return rows
    .filter((row) => {
      const paymentId = String(row.dlocal_payment_id || '');
      const provider = String(row.details?.provider || '');
      return paymentId.startsWith('DP-') || provider === 'pagarme';
    })
    .map((row) => ({ email: row.email, account_id: row.account_id }));
}

export async function listPaidDlocalCheckoutEmails({
  sinceIso,
  untilIso
} = {}) {
  if (!sbConfigured()) return [];
  const rows = await sbSelectWhere('dlocal_checkout_intents', {
    eq: { status: 'paid' },
    like: { dlocal_payment_id: 'DP-*' },
    ...(sinceIso ? { gte: { created_at: sinceIso } } : {}),
    ...(untilIso ? { lt: { created_at: untilIso } } : {}),
    order: 'created_at.asc',
    limit: 5000
  });
  return [...new Set(rows.map((row) => normalizeSubscriberEmail(row.email)).filter(Boolean))];
}

export async function fetchPagarmeSubscriberSnapshot({ occupiedEmails = [] } = {}) {
  ...
  const today = brtToday();
  const occupied = new Set((occupiedEmails || []).map(normalizeSubscriberEmail).filter(Boolean));
  const sinceDay = shiftDays(today, -PAGARME_ACTIVE_DAYS);
  const untilDay = shiftDays(today, 2);
  let orders = [];
  let subscriptions = [];
  let pages = 0;

  // Intents sozinhas perdem quem pagou na Stone e o checkout nao gravou.
  // A API sozinha corta em 80 paginas se a janela for unica. Junta as duas
  // e fatia a listagem — senao quem saiu da Guru some do total.
  if (pagarmeConfigured()) {
    const listed = await listPagarmeOrdersChunked({
      sinceDay,
      untilDay,
      status: 'paid'
    });
  ...
  if (sbConfigured()) {
    const intents = await listPagarmeAssinaturaIntents({
      sinceIso: `${sinceDay}T00:00:00.000Z`,
      untilIso: `${untilDay}T00:00:00.000Z`,
      statuses: ['paid']
    });
    orders = mergePagarmeOrdersById(
      orders,
      collapsePagarmeRepeatIntents(intents).map(intentAsPagarmeOrder)
    );
  }

  if (pagarmeConfigured()) {
    const subs = await listAllPagarmeSubscriptions({ status: 'active', maxPages: 20 });
    pages += subs.pages;
  ...
  const visible = orders.filter((order) => {
    const email = pagarmePartyEmail(order);
    return email && !occupied.has(email);
  });
  ...
  return {
    ...buildPagarmeSubscriberSnapshot({ orders: visible, subscriptions: visibleSubs }),
    pages
  };
}
