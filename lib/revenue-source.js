...
import { getPagouTransaction, listAllPagouSubscriptions, listPagouTransactions, pagouConfigured } from './pagou.js';
...
const PAGOU_ONE_SHOT_KINDS = new Set(['one_shot', 'oneshot', 'avulso', 'upgrade']);
...
export function pagouSubscriptionIsRecurring(sub) {
  const status = String(sub?.status || '').toLowerCase();
  if (status !== 'active') return false;
  const kind = String(sub?.metadata?.kind || '').toLowerCase();
  if (PAGOU_ONE_SHOT_KINDS.has(kind)) return false;
  const method = String(sub?.paymentMethod || sub?.payment_method || '').toLowerCase();
  const mandate = String(sub?.metadata?.provider_recurring?.mandate_status || '').toLowerCase();
  if (method === 'pix_automatic') return mandate === 'approved';
  return Boolean(sub?.interval);
}

export function buildPagouSubscriberSnapshot({ subscriptions = [], transactions = [] } = {}) {
  const recurringSubs = subscriptions.filter(pagouSubscriptionIsRecurring);
  const recurringEmails = new Set(recurringSubs.map(pagouPartyEmail).filter(Boolean));
  const prepaidEmails = new Set();
  for (const tx of transactions) {
    const email = pagouPartyEmail(tx);
    if (!email || recurringEmails.has(email)) continue;
    prepaidEmails.add(email);
  }
  return {
    count: recurringSubs.length + prepaidEmails.size,
    recurring: recurringSubs.length,
    prepaid: prepaidEmails.size
  };
}
