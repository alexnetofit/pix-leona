...
export function isPagarmeLeonaOrder(order = {}) {
  // Só o checkout da /assinatura (code leona-…). A Guru também liquida na
  // mesma conta Stone; UUID da transação Guru ou "Leona Flow" sem prefixo
  // leona- não vale — já entra no card da Guru.
  const code = String(order?.code || order?.order?.code || '');
  return /^leona-/i.test(code);
}
