...
/**
 * O payload de um webhook só serve de prova para o checkout que ele cita.
 * Sem isso um pedido pago liberaria qualquer intent pendente que passasse
 * pelo reconcile junto com ele.
 */
export function pagarmePayloadIsAboutCheckout(payload, paymentId) {
  const id = String(paymentId || '');
  if (!id) return false;
  return extractPagarmeOrderId(payload) === id
    || extractPagarmePaymentLinkId(payload) === id;
}

async function loadAssinaturaPaidProof(paymentId, payload = {}) {
  const orderId = (/^or_/i.test(String(paymentId)) && String(paymentId))
    || (pagarmePayloadIsAboutCheckout(payload, paymentId)
      ? extractPagarmeOrderId(payload)
      : null);
  let order = null;
  if (orderId) {
...
/**
 * Confere cada intent pendente contra a própria Pagar.me. Não recebe payload
 * de webhook de propósito: o evento que disparou o reconcile é de outro
 * checkout e não pode servir de prova de pagamento para estas intents.
 */
