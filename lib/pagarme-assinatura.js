...
export async function processPagarmeAssinaturaPaid(paymentLinkId, {
  ...
  // Só grava o evento DEPOIS da Leona ficar active. Se marcar antes e o POST
  // na API falhar (Cloudflare 1010, timeout), o replay vira duplicate e a
  // conta fica presa para sempre.
  ...
  if (activated.ok) {
    await rememberEvent(eventId, accountId, { qty, source });
  }
...
export function parseLeonaPagarmeOrderCode(code) {
  const m = String(code || '').match(/^leona-(\d+)-(\d+)-(sub|prorata)(?:-|$)/i);
  if (!m) return null;
  return { accountId: m[1], qty: Number(m[2]), kind: m[3].toLowerCase() };
}
