...
export const TRILHA_REVENUE_GRANTS = [
  ...
  {
    accountId: '1618',
    email: 'ronaldinholuchetti@gmail.com',
    extra: 196_981.35
  },
  {
    accountId: '10726',
    email: 'joaolucasmarketingytofc@gmail.com',
    extra: 2_000_000
  },
  {
    accountId: '830',
    email: '44prodcontato@gmail.com',
    extra: 500_000
  },
  {
    accountId: '1508',
  ...
  {
    accountId: '121',
    email: 'gabsstrabalho@gmail.com',
    extra: 1_000_000
  },
  {
    accountId: '7468',
    email: 'luidemorais73@gmail.com',
    extra: 200_000
  },
  {
    // PDFs fecham em R$ 96.266,68 (Kiwify R$ 61.923,93 + InfinitePay R$ 34.342,75).
    // O extra é a diferença sobre o lifetime da API (R$ 57.182,98 em 23/09/26),
    // para o dashboard terminar nesse total — não soma os 96 mil por cima.
    accountId: '8691',
...
/** Unidades extras no preço da trilha (não no custo de produção). */
export const TRILHA_CHEAP_EXTRA_GRANTS = [
  {
    accountId: '2884',
    email: 'yuriautomacoes@gmail.com',
    qty: 1
  }
];

export function trilhaCheapExtraGrantQty(accountId, email) {
  const id = normId(accountId);
  const emailNorm = normEmail(email);
  if (!id || !emailNorm) return 0;
  const grant = TRILHA_CHEAP_EXTRA_GRANTS.find((row) => (
    normId(row.accountId) === id && normEmail(row.email) === emailNorm
  ));
  const qty = Number(grant?.qty);
  return Number.isFinite(qty) && qty > 0 ? Math.floor(qty) : 0;
}

export function remainingCheapExtraUnits(accountId, email, extrasPurchased = 0) {
  const granted = trilhaCheapExtraGrantQty(accountId, email);
  const used = Math.max(0, Number(extrasPurchased) || 0);
...
export function buildTrilhaPayload({
  ...
  const prizes = TRILHA_PRIZES.map((prize) => {
    ...
    const cheapExtra = acquired && Number(cheapExtraRemaining) > 0;
    const displayCents = acquired
      ? (cheapExtra ? prize.priceCents : (prize.extraUnitCents || prize.priceCents))
      : prize.priceCents;
    ...
    return {
      ...
      cheap_extra: cheapExtra,
      displayCents,
      priceFormatted: formatBrl(displayCents / 100),
      extraPriceFormatted: formatBrl((prize.extraUnitCents || 0) / 100),
      cta: acquired
        ? (cheapExtra ? 'Resgatar extra' : 'Adicionar extra')
        : canAnticipate ? 'Antecipar premiação' : 'Resgatar',
      unlocked,
  ...
  return {
    ...
    cheap_extra_remaining: Math.max(0, Number(cheapExtraRemaining) || 0)
  };
}
