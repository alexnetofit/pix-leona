...
export function buildTrilhaPayload({ accountId, profile, revenueValue, revenueSource, redeemEligibility }) {
  ...
  const prizes = TRILHA_PRIZES.map((prize) => {
    ...
    let status = 'locked';
    if (unlocked && canRedeem) status = 'available';
    ...
    return {
      ...
      unlocked,
      redeem_blocked: unlocked && !canRedeem,
      status
    };
  });
