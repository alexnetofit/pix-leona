...
function appendPostgrestFilters(params, filters = {}) {
  const operators = ['eq', 'neq', 'gt', 'gte', 'lt', 'lte', 'like', 'ilike', 'is'];
  for (const operator of operators) {
    for (const [column, value] of Object.entries(filters[operator] || {})) {
      params.append(column, `${operator}.${value}`);
    }
  }
  for (const [column, values] of Object.entries(filters.in || {})) {
    params.append(column, `in.(${values.map(value => String(value)).join(',')})`);
  }
  for (const [column, value] of Object.entries(filters.cs || {})) {
    const encoded = typeof value === 'string' ? value : JSON.stringify(value);
    params.append(column, `cs.${encoded}`);
  }
}

/**
 * UPSERT por uma ou mais colunas de conflito.
 */
export async function sbUpsert(table, payload, {
  onConflict,
  ...
  const body = await r.json().catch(() => ({}));
  if (!r.ok) {
    throw new Error(`sbUpsert ${table} falhou (${r.status}): ${JSON.stringify(body).slice(0, 300)}`);
  }
  if (single && Array.isArray(body)) return body[0];
  return body;
}

/**
 * PATCH com operadores condicionais PostgREST. Util para claims otimistas.
 */
export async function sbUpdateWhere(table, filters, patch, opts = {}) {
  const creds = getCreds();
  if (!creds) throw new Error('SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY nao configurado');

  const params = new URLSearchParams();
  appendPostgrestFilters(params, filters);

  const r = await fetch(`${creds.url}/rest/v1/${encodeURIComponent(table)}?${params.toString()}`, {
    method: 'PATCH',
    headers: defaultHeaders(creds.key, { 'Prefer': 'return=representation' }),
    body: JSON.stringify(patch)
  });

  const body = await r.json().catch(() => ({}));
  if (!r.ok) {
    throw new Error(`sbUpdateWhere ${table} falhou (${r.status}): ${JSON.stringify(body).slice(0, 300)}`);
  }
  if (opts.single !== false && Array.isArray(body)) return body[0];
  return body;
}
...
/**
 * DELETE com operadores condicionais PostgREST.
 */
export async function sbDeleteWhere(table, filters = {}) {
  const creds = getCreds();
  if (!creds) throw new Error('SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY nao configurado');
  ...
  const r = await fetch(`${creds.url}/rest/v1/${encodeURIComponent(table)}?${params.toString()}`, {
    method: 'DELETE',
    headers: defaultHeaders(creds.key, { Prefer: 'return=minimal' })
  });

  if (!r.ok) {
    const body = await r.json().catch(() => ({}));
    throw new Error(`sbDeleteWhere ${table} falhou (${r.status}): ${JSON.stringify(body).slice(0, 300)}`);
  }
}

/**
 * SELECT com operadores condicionais adicionais ao sbSelect.
 */
export async function sbSelectWhere(table, query = {}) {
  const creds = getCreds();
  if (!creds) throw new Error('SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY nao configurado');

  const params = new URLSearchParams();
  params.set('select', query.select || '*');
  appendPostgrestFilters(params, query);
  if (query.order) params.set('order', query.order);
  if (query.limit) params.set('limit', String(query.limit));
  ...
  const body = await r.json().catch(() => ({}));
  if (!r.ok) {
    throw new Error(`sbSelectWhere ${table} falhou (${r.status}): ${JSON.stringify(body).slice(0, 300)}`);
  }
  return Array.isArray(body) ? body : [];
}
...
export async function sbRpc(functionName, args = {}) {
  ...
  if (!creds) throw new Error('SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY nao configurado');
  const r = await fetch(
    `${creds.url}/rest/v1/rpc/${encodeURIComponent(functionName)}`,
    {
      method: 'POST',
      headers: defaultHeaders(creds.key, { 'Prefer': 'return=representation' }),
  ...
  const body = await r.json().catch(() => ({}));
  if (!r.ok) {
    throw new Error(`sbRpc ${functionName} falhou (${r.status}): ${JSON.stringify(body).slice(0, 300)}`);
  }
  return body;
}
