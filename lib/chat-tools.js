...
async function cancelGuruSubscription({ subscription_id }) {
  ...
  // Subscription IDs Guru sao UUID v4 (ex: a1a9f046-b7ef-4a9b-977b-e7668ab20325).
  // IDs com prefixo sub_/sub_xxx sao Stripe ou Paddle e batem 404 na Guru —
  // recusamos antes de chamar pra LLM nao confundir gateways.
  const uuidRe = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  if (!uuidRe.test(subscription_id)) {
    return fail(
      `subscription_id "${subscription_id}" nao parece id Guru (UUID). ` +
      `IDs com prefixo "sub_" sao Stripe/Paddle. Use o campo "id" retornado por search_guru_by_email.`
    );
  }
