...
async function lookupLeonaByEmail(email) {
  ...
  return ok({
    ...
    ...leonaConnectionsAndTokens(p)
  });
}

function leonaConnectionsAndTokens(profile) {
  const rows = Array.isArray(profile?.instances) ? profile.instances : [];
  return {
...
async function getPaddleTransaction(transaction_id) {
  ...
  // Paddle so aceita includes especificos aqui (adjustments,
  // adjustments_totals, available_payment_methods, business, customer,
  // discount, address). 'subscription' nao e include valido.
  const r = await fetch(`${PADDLE_BASE}/transactions/${transaction_id}?include=customer,address,adjustments`, {
    headers: paddleHeaders()
  });
  if (!r.ok) return fail(`Paddle ${r.status}`);
...
// DEBUG: bate na API Guru sem normalizar nem filtrar por produto.
// Util pra investigar quando subs nao aparecem onde deveriam.
async function debugGuruRaw(email) {
  const token = process.env.GURU_TOKEN;
  if (!token) return fail('GURU_TOKEN nao configurado');
  const headers = { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json', 'User-Agent': 'n8n' };
  try {
    const cRes = await fetch(`https://digitalmanager.guru/api/v2/contacts?email=${encodeURIComponent(email)}&limit=20`, { headers });
    const cBody = cRes.ok ? await cRes.json() : null;
    const contact = (cBody?.data || []).find(c => c.email?.toLowerCase() === email.toLowerCase());
    if (!contact) return ok({ contact_found: false, raw_contact_response: cBody });

    const sRes = await fetch(`https://digitalmanager.guru/api/v2/subscriptions?contact_id=${contact.id}&limit=50`, { headers });
...
async function searchGuruByEmail(email, only_active = false) {
  ...
  try {
    ...
    // O default de findGuruSubscriptionsByEmail é onlyActive:true. Pra
    // realmente trazer todas (status canceladas, expiradas, etc) precisa
    // passar explicito { onlyActive: false }.
    const subs = only_active
      ? await findGuruActiveSubscriptionsByEmail(email, token)
      : await findGuruSubscriptionsByEmail(email, token, { onlyActive: false });

    return ok({
      contact: contact || null,
      // Mantem os mesmos nomes de campo que o normalizador de lib/guru.js
      // ja produz, pra nao introduzir mais um mapeamento divergente.
      subscriptions: (subs || []).map(s => ({
        id: s.id,
        subscription_code: s.subscription_code,
        status: s.status,             // active / canceled / etc
        status_at: s.status_at,        // ISO 8601 - quando entrou nesse status
        offer_id: s.offer_id,
        offer_name: s.offer_name,
        product_id: s.product_id,
        product_name: s.product_name,
        payment_method: s.payment_method,
        cycle_start: s.cycle_start,    // YYYY-MM-DD
        cycle_end: s.cycle_end,        // YYYY-MM-DD
        next_cycle: s.next_cycle,      // YYYY-MM-DD
        cancelled_at: s.cancelled_at   // ISO 8601 (ou null)
      })),
...
async function cancelGuruSubscription({ subscription_id, cancel_at_cycle_end, comment }) {
  const token = process.env.GURU_TOKEN;
  if (!token) return fail('GURU_TOKEN nao configurado');
  if (!subscription_id) return fail('subscription_id obrigatorio');

  // Guru aceita tanto o UUID interno (campo `id`) quanto o subscription_code
  // (campo `subscription_code`, formato sub_xxx) no path. Mas IDs Stripe/Paddle
  // tambem comecam com sub_ — pra evitar mandar id de gateway errado, exigimos
  // que seja UUID Guru OU subscription_code retornado pelas tools de busca Guru.
  const uuidRe = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  const guruCodeRe = /^sub_[A-Za-z0-9]{14,}$/;
  if (!uuidRe.test(subscription_id) && !guruCodeRe.test(subscription_id)) {
    return fail(
      `subscription_id "${subscription_id}" nao parece id Guru. ` +
      `Use o "id" (UUID) ou o "subscription_code" (sub_xxx) retornado por search_guru_by_email.`
    );
  }

  try {
    const result = await guruCancelSub(subscription_id, token, {
      cancel_at_cycle_end: cancel_at_cycle_end === true,
      comment: comment || 'Cancelamento via suporte Leona'
    });
    if (!result.ok) {
      return fail(
        `Guru recusou o cancelamento (status ${result.status ?? 'desconhecido'}): ` +
        `${result.body?.message || result.error || 'sem detalhes'}`,
        { subscription_id, gateway_response: result }
      );
    }
    return ok({ subscription_id, cancelled: true, gateway_response: result });
  } catch (e) {
...
/**
 * Liberacao manual: pega a sub Paddle ATIVA do cliente e empurra
 * { status: 'active', starter_instances: <quantity> } pro Leona.
 * Util quando o webhook nao disparou (Paddle Notification mal configurada,
 * Vercel offline na hora, etc) e o cliente pagou mas nao foi liberado.
 */
async function syncPaddleToLeona({ account_id, subscription_id }) {
  const leonaToken = process.env.LEONA_BILLING_TOKEN;
  if (!leonaToken) return fail('LEONA_BILLING_TOKEN nao configurado');
  if (!process.env.PADDLE_API_KEY) return fail('PADDLE_API_KEY nao configurado');
  if (!account_id) return fail('account_id obrigatorio');

  let sub;
  if (subscription_id) {
    const r = await fetch(`${PADDLE_BASE}/subscriptions/${subscription_id}`, { headers: paddleHeaders() });
    if (!r.ok) return fail(`Paddle ${r.status} buscando sub ${subscription_id}`);
    sub = (await r.json()).data;
  } else {
    const profile = await getLeonaBillingProfile(account_id, leonaToken);
    const email = profile?.user?.email;
    if (!email) return fail(`account_id ${account_id} nao tem email no Leona`);

    const cusRes = await fetch(`${PADDLE_BASE}/customers?email=${encodeURIComponent(email)}`, { headers: paddleHeaders() });
    if (!cusRes.ok) return fail(`Paddle ${cusRes.status} buscando customer`);
    const customers = (await cusRes.json()).data || [];
    const cust = customers.find(c => c.email?.toLowerCase() === email.toLowerCase());
    if (!cust) return fail(`Sem customer Paddle pro email ${email}`);

    const subsRes = await fetch(`${PADDLE_BASE}/subscriptions?customer_id=${cust.id}&status=active&per_page=10`, { headers: paddleHeaders() });
    if (!subsRes.ok) return fail(`Paddle ${subsRes.status} buscando subs`);
    const subs = (await subsRes.json()).data || [];
    if (subs.length === 0) return fail('Sem sub Paddle ativa pra esse cliente');
    sub = subs[0];
  }

  const qty = (sub.items || []).reduce((acc, it) => acc + (Number(it.quantity) || 0), 0);
  if (qty <= 0) return fail('Sub Paddle sem quantity > 0');

  // due_date: Leona quer YYYY-MM-DD. next_billed_at vem ISO completo.
  const nextBilled = sub.next_billed_at;
  const dueDate = nextBilled ? String(nextBilled).slice(0, 10) : null;
  ...
  const result = await updateLeonaBillingProfile(account_id, payload, leonaToken);
  if (!result.ok) return fail(`Falha ao atualizar Leona: ${JSON.stringify(result.body || result.error)}`);
  ...
  return ok({
    ...
    leona_response: result.body,
    note: 'Cliente liberado manualmente. Recomenda-se verificar por que o webhook-paddle nao processou esse evento.'
  });
}

async function adjustLeonaTokenBalance({ account_id, action, amount, note }) {
  const token = process.env.LEONA_BILLING_TOKEN;
  if (!token) return fail('LEONA_BILLING_TOKEN nao configurado');
  if (!account_id) return fail('account_id obrigatorio');
  const result = await adjustLeonaTokens(account_id, { action, amount, note }, token);
  if (!result.ok) {
    return fail(result.body?.error || result.error || `Leona HTTP ${result.status || '?'}`);
  }
  return ok({
    account_id: result.body?.account_id ?? account_id,
    action,
    amount,
    tokens: result.body?.tokens || null
  });
}
...
export const TOOLS = [
  ...
  },
  {
    type: 'function',
    function: {
      name: '_debug_guru_raw',
      description: 'DEBUG: retorna lista crua de subscriptions Guru de um contato (sem filtro de produto, sem filtro de status). Use pra investigar quando subs nao aparecem nas outras tools.',
      parameters: { type: 'object', properties: { email: { type: 'string' } }, required: ['email'] }
    }
  },
  {
    ...
    function: {
      ...
      description: 'ACAO: Cancela uma subscription Guru via POST /subscriptions/:id/cancel. Use APENAS quando o suporte pedir explicitamente ("cancele a Guru", "remove essa sub", etc). subscription_id pode ser o UUID (campo "id") OU o subscription_code (campo "subscription_code", formato sub_xxx) — ambos retornados por search_guru_by_email/_debug_guru_raw. Default: cancela imediato (cancel_at_cycle_end=false) com comentario generico. So confie em "cancelled: true"; se vier "ok: false", NAO afirme ao cliente que a sub foi cancelada.',
      parameters: {
        type: 'object',
        properties: {
          subscription_id: {
            type: 'string',
            description: 'UUID Guru ("id") ou subscription_code ("sub_xxx"). Nao confundir com ids Stripe/Paddle.'
          },
          cancel_at_cycle_end: {
            type: 'boolean',
            description: 'true = cancela no fim do ciclo (cliente continua usando ate vencer). false = cancela imediato. Default: false.'
          },
          comment: {
            type: 'string',
            description: 'Motivo registrado no painel Guru. Default: "Cancelamento via suporte Leona".'
          }
        },
  ...
  },
  {
    type: 'function',
    function: {
      name: 'sync_paddle_to_leona',
      description: 'ACAO: Forca a liberacao manual de uma conta Leona com base na sub Paddle ATIVA do cliente. Use quando o cliente pagou na Paddle mas o webhook nao liberou no Leona (status fica past_due, instances=0). Pega a quantity da sub e seta { status: active, starter_instances: <qty> } no Leona. Use APENAS quando o suporte pedir explicitamente.',
      parameters: {
        type: 'object',
        properties: {
          account_id: { type: 'integer', description: 'Account ID Leona da conta a liberar' },
          subscription_id: { type: 'string', description: 'Sub Paddle especifica (sub_xxx). Se omitido, busca a primeira sub ativa do cliente.' }
        },
        required: ['account_id']
      }
    }
  },
  {
    type: 'function',
    function: {
      name: 'adjust_leona_tokens',
      description: 'ACAO: Credita (add) ou debita (remove) tokens da carteira Leona via POST /accounts/:id/tokens. Nao altera plano, vencimento nem conexoes. Use APENAS quando o suporte pedir explicitamente. amount tem que ser positivo. remove maior que o saldo disponivel nao debita.',
      parameters: {
        type: 'object',
        properties: {
          account_id: { type: 'integer', description: 'Account ID Leona' },
          action: { type: 'string', enum: ['add', 'remove'], description: 'add credita, remove debita' },
          amount: { type: 'number', description: 'Quantidade positiva de tokens' },
          note: { type: 'string', description: 'Motivo para auditoria, ate 500 caracteres' }
        },
        required: ['account_id', 'action', 'amount']
      }
    }
  }
];
...
export const TOOL_IMPLEMENTATIONS = {
  ...
  _debug_guru_raw: ({ email }) => debugGuruRaw(email),
  cancel_guru_subscription: cancelGuruSubscription,
  sync_paddle_to_leona: syncPaddleToLeona,
  create_paddle_renewal_checkout: createPaddleRenewalCheckout,
  adjust_leona_tokens: adjustLeonaTokenBalance
};
