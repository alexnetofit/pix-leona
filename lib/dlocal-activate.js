 ...
 * Usado pelo webhook e pelo cron de reconciliação — o webhook da Go
 * em assinatura (plan subscribe_url) frequentemente não chega.
 */
import { logAssinaturaEvent } from './assinatura-log.js';
import { findLeonaAccountByEmail, getLeonaBillingProfile } from './leona.js';
import { dueDatePlusDays } from './leona-pricing.js';
import { isAffiliateReversal, notifyAffiliatesPagou } from './notify-affiliates.js';
import { sbConfigured, sbInsert, sbSelect, sbSelectWhere, sbUpdate } from './supabase.js';
...
export async function processDlocalPaidPayment(payment, {
  ...
  if (!accountId && email) {
    const found = await findLeonaAccountByEmail(email, process.env.LEONA_BILLING_TOKEN);
    if (found?.account_id) accountId = String(found.account_id);
  }

  if (!accountId || !qty) {
    console.error('dlocal-activate: sem account/qty', { paymentId, email, orderId, ref, intent: intent?.id });
    logAssinaturaEvent(req, {
      action: 'dlocal_webhook_unresolved',
      provider: 'dlocal',
      email,
      account_id: accountId,
      details: { payment_id: paymentId, order_id: orderId, ref, intent_id: intent?.id || null, source }
    });
  ...
  if (!profile) {
    console.error('dlocal-activate: conta nao encontrada', accountId);
    return { processed: false, error: `conta ${accountId} não encontrada`, payment_id: paymentId };
  }
